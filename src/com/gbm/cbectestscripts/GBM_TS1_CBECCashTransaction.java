...
public class GBM_TS1_CBECCashTransaction {
	...
	@Test (description="Creates and validates the CBEC - Cash transaction mode")
	public static void CBECCashTransaction() 
	{
		try
		{
			...
			// Navigate to CBEC Transaction Maintenance page for adding a transaction
			...
			if(!bStatus)
			{
				Reporting.logResults("Fail", "Failed to open 'create transaction' page in CBEC", "Navigation failed. Error Message:" + Messages.errorMsg);
				Assert.fail( Messages.errorMsg);	
			}
			Reporting.logResults("Pass", " Navigate to CBEC create transaction page.","Transaction Maintenance page is opened to create new transaction.");

			// Create CBEC Cash transaction by entering data in all mandatory fields
			...
			objTransactionDetails.put("TransactionId", sCBCETransactionId);
			...
			// Verify the CBEC- Cash transaction

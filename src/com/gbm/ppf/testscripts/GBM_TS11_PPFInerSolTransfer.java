...
public class GBM_TS11_PPFInerSolTransfer {
	...
	static String sTestCaseName = "PPF_IINTERSOL_TRANSFER_TC11";
	...
	@Test
	public static void PPFInterSolTransferTransaction() 
	{
		try
		{
			...
			{
				Reporting.logResults("Fail", "Failed to create PPF Transfer Transaction", "Transaction creation failed. Error Message:" + Messages.errorMsg);
				Assert.fail( Messages.errorMsg);	
			}
			Reporting.logResults("Pass", "Create PPF Transfer Transaction.","Transaction created successfully."+Messages.appErrorMsg);
			...
			// Navigate to CBDT Transaction Maintenance page for adding a transaction
			bStatus = ApplicationFunc.navigateToTransactionMaintenance(objTransactionDetails.get("ScenarioType"), objTransactionDetails.get("TransactionMaintanceType"));
			if(!bStatus)
			{
				Reporting.logResults("Fail", "Failed to open create transaction page", "Navigation failed. Error Message:" + Messages.errorMsg);
				Assert.fail( Messages.errorMsg);	

...
public class GBM_TS1_SalesTaxCashTransaction {
	...
	@Test (description="Creates and validates the SalesTax - Cash transaction mode")
	public static void salesTaxCashTransaction() 
	{
		try
		{
			...
			if(!bStatus)
			{
				Reporting.logResults("Fail", "Failed to open 'create transaction' page in StateTax", "Navigation failed. Error Message:" + Messages.errorMsg);
				Assert.fail( Messages.errorMsg);	
			}
			Reporting.logResults("Pass", " Navigate to StateTax create transaction page.","Transaction Maintenance page is opened to create new transaction.");

			// Create State Tax Cash transaction by entering data in all mandatory fields
